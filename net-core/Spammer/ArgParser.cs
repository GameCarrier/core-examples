...
namespace GameCarrier.Examples
{
    internal class SpammerArgParser
    {
        ...
        private readonly Queue<string> argsTail;
        ...
        private void ParseUri(string uri)
        {
            ...
            var proto = m.Groups[1].Value switch
            {
                "" => ConnectionProtocol.WebSocketSecure,
                "wss://" => ConnectionProtocol.WebSocketSecure,
                "ws://" => ConnectionProtocol.WebSocket,
                _ => throw new BadArguments($"Invalid proto {protoStr}"),
            };
        ...
        bool TryIntArg(string arg, string name, ref int variable)
        {
            ...
            SetInt(name, arg[eqPos..], ref variable);
            return true;
        }
            ...
            for (int i = 1; i < arg.Length; ++i)
            {
                ...
                switch (ch)
                {
                    ...
                    case 'h':
                        ...
                        break;
                    case 'k':
                        ReadShortValue($"-{ch}", arg[(i + 1)..], ref BaseMsgSizeInKb);
                        return;
                    case 'r':
                        ReadShortValue($"-{ch}", arg[(i + 1)..], ref RelaxTime);
                        return;
                    case 's':
                        ReadShortValue($"-{ch}", arg[(i + 1)..], ref SleepTime);
                        return;
                    default:
        ...
        public void Dump()
        {
            ...
            Queue<int> uriCounts = new(UriCounts);
