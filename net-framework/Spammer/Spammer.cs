...
namespace Spammer
{
    ...
    {
        public static SpammerArgParser ArgParser = new SpammerArgParser();
        public static Random Rnd = new Random();

        private static readonly Logger log = new Logger("Spammer");
        private static readonly List<SpammerClient> clients = new List<SpammerClient>();

        private class SpammerClient : GcClient
        {
            private static readonly Logger log = new Logger("Client");
            ...
            public void DoAction()
            {
                ...
                int sz = 128 + 1024 * ArgParser.BaseMsgSizeInKb + Rnd.Next(0, 1024);
        ...
        static void RunMode(GcClientMode mode)
        {
            try
            {
                ...
                clients.Clear();
                Queue<int> uriCounts = new Queue<int>(ArgParser.UriCounts);
        ...
        static int Main(string[] args)
        {
            try
            {
                int? exitCode = ArgParser.Parse(args);
                if (exitCode != null)
                {
                    return (int)exitCode;
                }
